...
func TestLsTool_Run(t *testing.T) {
	...
	t.Run("handles empty path parameter", func(t *testing.T) {
		...
		assert.NotContains(t, response.Content, "error")
	})
	...
	t.Run("handles relative path", func(t *testing.T) {
		t.Skip("Skipping test due to config/working directory interaction issues")
		...
		// Reinitialize config with the new working directory
